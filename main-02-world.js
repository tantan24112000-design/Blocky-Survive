...
function generateWorld(seed=worldSeed){worldSeed=normalizeSeedText(seed);const oldRandom=Math.random;const rng=seededRandomFactory(worldSeed);Math.random=rng;try{
  ...
  for(let x=0;x<WW;x++){
    ...
    for(let y=0;y<WH;y++){
      ...
      else if(r<0.141&&y>sh+42)world[y*WW+x]=B.CRYSTAL;
      else if(r<0.143&&y>sh+48)world[y*WW+x]=B.BASALT;
    }
  ...
  for(let y=8;y<WH-5;y++)for(let x=3;x<WW-3;x++){
    if(getB(x,y)===B.DIRT&&(getB(x-1,y)===B.WATER||getB(x+1,y)===B.WATER||getB(x,y-1)===B.WATER)&&Math.random()<0.55)
      setB(x,y,B.MUD);
  }
...
function getSaveInfo(slot){try{const d=JSON.parse(localStorage.getItem('ps_slot_'+slot));return d?{time:d.timestamp||0,mode:d.mode||'survival',seed:d.seed||''}:null;}catch(e){return null;}}
let autoSaveTimer=0;
...
let worldMode='survival';
let worldSeed='';
let pendingWorldCreateSlot=-1;
let pendingWorldMode='survival';
let pendingWorldSeed='';
function normalizeSeedText(seed){const s=(seed===undefined||seed===null)?'' : String(seed).trim();return s||('seed-'+Date.now().toString(36));}
function hashSeedText(seed){let h=2166136261>>>0;for(let i=0;i<seed.length;i++){h^=seed.charCodeAt(i);h=Math.imul(h,16777619);}return h>>>0;}
function seededRandomFactory(seed){let t=hashSeedText(seed)||1;return function(){t^=t<<13;t^=t>>>17;t^=t<<5;return((t>>>0)/4294967296);};}
function getCreativeBlockKeys(){return Object.keys(IT).filter(k=>IT[k]&&IT[k].block!==undefined).sort((a,b)=>itemName(a).localeCompare(itemName(b),'vi'));}
const CREATIVE_BLOCK_KEYS=getCreativeBlockKeys();
...
function respawnPlayer(){player.health=player.maxHealth;player.hunger=player.maxHunger;player.isDead=false;chatAnnouncedDeath=false;player.x=spawnBX*BS-13;player.y=spawnBY*BS;player.vy=0;player.vx=0;}
...
let bookFlipAnim=0,bookFlipDir=1,bookFlipFromPage=0;
const swordSlash={active:false,timer:0,maxTimer:380,dir:1,x:0,y:0,trail:[]};
const BOOK_PAGES=[
  ...
  {title:'Vật Liệu Mới I',recipes:['MOSSY_COBBLE','HAY_BALE','TERRACOTTA']},
  {title:'Vật Liệu Mới II',recipes:['BASALT',null,null]},
];
...
function handleCraftClick(mx,my,isRight){if(worldMode==='creative'){const slot=getCreativeSlotAt(mx,my);if(!slot)return;sndClick();if(slot.type==='palette'){const key=CREATIVE_BLOCK_KEYS[slot.idx];if(key){inv[hotIdx]={key,count:64};}return;}const slotData=inv[slot.idx];if(isRight){if(cursorItem){if(slotData){if(slotData.key===cursorItem.key&&slotData.count<64){slotData.count++;cursorItem.count--;if(cursorItem.count<=0)cursorItem=null;}}else{inv[slot.idx]={key:cursorItem.key,count:1};cursorItem.count--;if(cursorItem.count<=0)cursorItem=null;}}else if(slotData&&slotData.count>0){const half=Math.ceil(slotData.count/2);cursorItem={key:slotData.key,count:half};slotData.count-=half;if(slotData.count<=0)inv[slot.idx]=null;}}else{if(cursorItem){if(slotData){if(slotData.key===cursorItem.key){const sp=64-slotData.count,tr=Math.min(sp,cursorItem.count);slotData.count+=tr;cursorItem.count-=tr;if(cursorItem.count<=0)cursorItem=null;}else{const tmp={key:slotData.key,count:slotData.count};inv[slot.idx]={key:cursorItem.key,count:cursorItem.count};cursorItem=tmp;}}else{inv[slot.idx]={key:cursorItem.key,count:cursorItem.count};cursorItem=null;}}else if(slotData){cursorItem={key:slotData.key,count:slotData.count};inv[slot.idx]=null;}}return;}const slot=getCraftSlotAt(mx,my);if(!slot)return;sndClick();if(slot.type==='output'){if(!isRight&&craftOutput)doCraft();return;}const slotData=slot.type==='grid'?craftGrid[slot.idx]:inv[slot.idx];if(isRight){if(cursorItem){if(slotData){if(slotData.key===cursorItem.key&&slotData.count<64){slotData.count++;cursorItem.count--;if(cursorItem.count<=0)cursorItem=null;}}else{const t=slot.type==='grid'?craftGrid:inv;t[slot.idx]={key:cursorItem.key,count:1};cursorItem.count--;if(cursorItem.count<=0)cursorItem=null;}}else if(slotData&&slotData.count>0){const half=Math.ceil(slotData.count/2);cursorItem={key:slotData.key,count:half};slotData.count-=half;if(slotData.count<=0){if(slot.type==='grid')craftGrid[slot.idx]=null;else inv[slot.idx]=null;}}}else{if(cursorItem){if(slotData){if(slotData.key===cursorItem.key){const sp=64-slotData.count,tr=Math.min(sp,cursorItem.count);slotData.count+=tr;cursorItem.count-=tr;if(cursorItem.count<=0)cursorItem=null;}else{const t=slot.type==='grid'?craftGrid:inv;const tmp={key:slotData.key,count:slotData.count};t[slot.idx]={key:cursorItem.key,count:cursorItem.count};cursorItem=tmp;}}else{const t=slot.type==='grid'?craftGrid:inv;t[slot.idx]={key:cursorItem.key,count:cursorItem.count};cursorItem=null;}}else if(slotData){cursorItem={key:slotData.key,count:slotData.count};if(slot.type==='grid')craftGrid[slot.idx]=null;else inv[slot.idx]=null;}}checkCraftRecipe();}
...
const CREATIVE_UI={px:0,py:0,pw:0,ph:0,paletteX:0,paletteY:0,invX:0,invY:0};
const CREATIVE_CELL=34,CREATIVE_GAP=3,CREATIVE_ROWS=7,CREATIVE_COLS=9;
function calcCreativeLayout(){const palW=CREATIVE_COLS*CREATIVE_CELL+(CREATIVE_COLS-1)*CREATIVE_GAP,invW=9*CS+8*CG;const palRows=Math.ceil(CREATIVE_BLOCK_KEYS.length/CREATIVE_COLS);const palH=palRows*CREATIVE_CELL+(palRows-1)*CREATIVE_GAP;const invH=3*CS+2*CG;const totalW=Math.max(palW,invW)+40;const totalH=30+palH+28+invH+46;CREATIVE_UI.px=(W-totalW)/2;CREATIVE_UI.py=(H-totalH)/2;CREATIVE_UI.pw=totalW;CREATIVE_UI.ph=totalH;CREATIVE_UI.paletteX=CREATIVE_UI.px+20;CREATIVE_UI.paletteY=CREATIVE_UI.py+38;CREATIVE_UI.invX=CREATIVE_UI.px+(totalW-invW)/2;CREATIVE_UI.invY=CREATIVE_UI.paletteY+palH+28;}
function getCreativeSlotAt(mx,my){calcCreativeLayout();for(let r=0;r<CREATIVE_ROWS;r++)for(let c=0;c<CREATIVE_COLS;c++){const idx=r*CREATIVE_COLS+c;if(idx>=CREATIVE_BLOCK_KEYS.length)break;const sx=CREATIVE_UI.paletteX+c*(CREATIVE_CELL+CREATIVE_GAP),sy=CREATIVE_UI.paletteY+r*(CREATIVE_CELL+CREATIVE_GAP);if(mx>=sx&&mx<sx+CREATIVE_CELL&&my>=sy&&my<sy+CREATIVE_CELL)return{type:'palette',idx};}for(let r=0;r<3;r++)for(let c=0;c<9;c++){const sx=CREATIVE_UI.invX+c*(CS+CG),sy=CREATIVE_UI.invY+r*(CS+CG);if(mx>=sx&&mx<sx+CS&&my>=sy&&my<sy+CS)return{type:'inv',idx:r*9+c};}return null;}
function drawCreativeUI(){calcCreativeLayout();ctx.fillStyle='rgba(0,0,0,0.72)';ctx.fillRect(0,0,W,H);ctx.fillStyle='#1a1a2e';ctx.fillRect(CREATIVE_UI.px,CREATIVE_UI.py,CREATIVE_UI.pw,CREATIVE_UI.ph);ctx.strokeStyle='#3a3a5a';ctx.lineWidth=2;ctx.strokeRect(CREATIVE_UI.px,CREATIVE_UI.py,CREATIVE_UI.pw,CREATIVE_UI.ph);ctx.lineWidth=1;ctx.fillStyle='#88ffaa';ctx.font='bold 14px "Courier New"';ctx.fillText('✦ Creative Inventory',CREATIVE_UI.px+16,CREATIVE_UI.py+24);ctx.fillStyle='#6a6a8a';ctx.font='11px "Courier New"';ctx.fillText('Pick any block above to put it in the selected slot',CREATIVE_UI.px+16,CREATIVE_UI.py+38);for(let r=0;r<CREATIVE_ROWS;r++)for(let c=0;c<CREATIVE_COLS;c++){const idx=r*CREATIVE_COLS+c;if(idx>=CREATIVE_BLOCK_KEYS.length)break;const key=CREATIVE_BLOCK_KEYS[idx];const sx=CREATIVE_UI.paletteX+c*(CREATIVE_CELL+CREATIVE_GAP),sy=CREATIVE_UI.paletteY+r*(CREATIVE_CELL+CREATIVE_GAP);ctx.fillStyle='#0c0c1c';ctx.fillRect(sx,sy,CREATIVE_CELL,CREATIVE_CELL);ctx.strokeStyle='#3a3a5a';ctx.strokeRect(sx,sy,CREATIVE_CELL,CREATIVE_CELL);drawCraftItem(sx,sy,{key,count:1},CREATIVE_CELL);if(inv[hotIdx]&&inv[hotIdx].key===key){ctx.strokeStyle='#88ffaa';ctx.lineWidth=2;ctx.strokeRect(sx+1,sy+1,CREATIVE_CELL-2,CREATIVE_CELL-2);ctx.lineWidth=1;}}ctx.fillStyle='#3a3a5a';ctx.fillRect(CREATIVE_UI.px+14,CREATIVE_UI.invY-12,9*CS+8*CG+8,1);ctx.fillStyle='#6a6a8a';ctx.font='11px "Courier New"';ctx.fillText('Inventory',CREATIVE_UI.invX,CREATIVE_UI.invY-3);for(let r=0;r<3;r++)for(let c=0;c<9;c++){const sx=CREATIVE_UI.invX+c*(CS+CG),sy=CREATIVE_UI.invY+r*(CS+CG),idx=r*9+c;ctx.fillStyle=idx===hotIdx?'#1a1a3e':'#0c0c1c';ctx.fillRect(sx,sy,CS,CS);ctx.strokeStyle=idx===hotIdx?'#6688cc':'#3a3a5a';ctx.strokeRect(sx,sy,CS,CS);const item=inv[idx];if(item)drawCraftItem(sx,sy,item,CS);}if(cursorItem)drawCraftItem(mouseX-20,mouseY-20,cursorItem,CS);ctx.fillStyle='#555';ctx.font='10px "Courier New"';ctx.fillText('Click: pick block | E/ESC: close',CREATIVE_UI.px+16,CREATIVE_UI.py+CREATIVE_UI.ph-12);}

function drawCraftUI(){if(worldMode==='creative'){drawCreativeUI();return;}calcCraftLayout();ctx.fillStyle='rgba(0,0,0,0.65)';ctx.fillRect(0,0,W,H);ctx.fillStyle='#1a1a2e';ctx.fillRect(CUI.px,CUI.py,CUI.pw,CUI.ph);ctx.strokeStyle='#3a3a5a';ctx.lineWidth=2;ctx.strokeRect(CUI.px,CUI.py,CUI.pw,CUI.ph);ctx.lineWidth=1;ctx.fillStyle='#88aaff';ctx.font='bold 14px "Courier New"';ctx.fillText('⚒ Crafting Table',CUI.px+16,CUI.py+24);for(let r=0;r<3;r++)for(let c=0;c<3;c++){const sx=CUI.gridX+c*(CS+CG),sy=CUI.gridY+r*(CS+CG);ctx.fillStyle='#0c0c1c';ctx.fillRect(sx,sy,CS,CS);ctx.strokeStyle='#3a3a5a';ctx.strokeRect(sx,sy,CS,CS);const item=craftGrid[r*3+c];if(item)drawCraftItem(sx,sy,item,CS);}ctx.fillStyle='#6a6a8a';const ax=CUI.arrowX,ay=CUI.arrowY+CS/2;ctx.beginPath();ctx.moveTo(ax,ay-6);ctx.lineTo(ax+20,ay);ctx.lineTo(ax,ay+6);ctx.closePath();ctx.fill();ctx.fillRect(ax+22,ay-3,16,6);ctx.fillStyle='#1a1a2e';ctx.fillRect(CUI.outX-2,CUI.outY-2,CS+4,CS+4);ctx.fillStyle='#0c0c1c';ctx.fillRect(CUI.outX,CUI.outY,CS,CS);ctx.strokeStyle=craftOutput?'#88ffaa':'#3a3a5a';ctx.strokeRect(CUI.outX,CUI.outY,CS,CS);if(craftOutput)drawCraftItem(CUI.outX,CUI.outY,{key:craftOutput.o,count:craftOutput.cnt},CS);ctx.fillStyle='#3a3a5a';ctx.fillRect(CUI.invX-4,CUI.invY-12,9*CS+8*CG+8,1);ctx.fillStyle='#6a6a8a';ctx.font='11px "Courier New"';ctx.fillText('Inventory',CUI.invX,CUI.invY-3);for(let r=0;r<3;r++)for(let c=0;c<9;c++){const sx=CUI.invX+c*(CS+CG),sy=CUI.invY+r*(CS+CG),idx=r*9+c;ctx.fillStyle=idx===hotIdx?'#1a1a3e':'#0c0c1c';ctx.fillRect(sx,sy,CS,CS);ctx.strokeStyle=idx===hotIdx?'#6688cc':'#3a3a5a';ctx.strokeRect(sx,sy,CS,CS);const item=inv[idx];if(item)drawCraftItem(sx,sy,item,CS);}if(cursorItem)drawCraftItem(mouseX-20,mouseY-20,cursorItem,CS);ctx.fillStyle='#555';ctx.font='10px "Courier New"';ctx.fillText('Left click: take/place | Right click: place 1 / take half | E/ESC: close',CUI.px+16,CUI.py+CUI.ph-12);}

const entities=[];let nextEntitySpawn=2000;
const arrows=[];
function updateEntities(dt){const s=dt/16.667;nextEntitySpawn-=dt;if(nextEntitySpawn<=0){nextEntitySpawn=4000+Math.random()*6000;const isDaytime=dayT>0.14&&dayT<0.86;if(entities.length<14){let bx=Math.floor(player.x/BS+(Math.random()>0.5?1:-1)*(18+Math.random()*14));if(bx<1||bx>=WW-1){nextEntitySpawn=1000;return;}let by=0;for(let y=0;y<WH;y++)if(solid(bx,y)){by=y;break;}if(by>0){const ey=by*BS;if(isDaytime){const sr=Math.random();if(sr<0.28)entities.push({type:'pig',x:bx*BS,y:ey-20,w:28,h:20,vx:(Math.random()>0.5?1:-1)*1.2,vy:0,health:15,maxHealth:15,dirTimer:0,hitFlash:0,attackTimer:0,walkPhase:0});else if(sr<0.50)entities.push({type:'cow',x:bx*BS,y:ey-24,w:34,h:22,vx:(Math.random()>0.5?1:-1)*0.9,vy:0,health:20,maxHealth:20,dirTimer:0,hitFlash:0,attackTimer:0,walkPhase:0});else if(sr<0.68)entities.push({type:'horse',x:bx*BS,y:ey-30,w:38,h:28,vx:(Math.random()>0.5?1:-1)*2.0,vy:0,health:25,maxHealth:25,dirTimer:0,hitFlash:0,attackTimer:0,walkPhase:0});else if(sr<0.84)entities.push({type:'cat',x:bx*BS,y:ey-14,w:14,h:14,vx:(Math.random()>0.5?1:-1)*1.0,vy:0,health:8,maxHealth:8,dirTimer:0,hitFlash:0,attackTimer:0,walkPhase:0});else entities.push({type:'dog',x:bx*BS,y:ey-16,w:20,h:16,vx:0,vy:0,health:12,maxHealth:12,dirTimer:0,hitFlash:0,attackTimer:0,walkPhase:0});}else{const sr=Math.random();if(sr<0.38)entities.push({type:'zombie',x:bx*BS,y:ey-48,w:22,h:48,vx:0,vy:0,health:40,maxHealth:40,dirTimer:0,dmgCD:0,hitFlash:0,attackTimer:0,walkPhase:0});else if(sr<0.68)entities.push({type:'creeper',x:bx*BS,y:ey-34,w:18,h:34,vx:0,vy:0,health:20,maxHealth:20,dirTimer:0,hitFlash:0,attackTimer:0,walkPhase:0,fuseTimer:0,fusing:false});else entities.push({type:'skeleton',x:bx*BS,y:ey-40,w:18,h:40,vx:0,vy:0,health:25,maxHealth:25,dirTimer:0,dmgCD:0,hitFlash:0,attackTimer:0,walkPhase:0,shootCD:0});}}}}
  ...
  if(e.type==='pig'||e.type==='cow'||e.type==='horse'){e.dirTimer-=dt;if(e.dirTimer<=0){const spd=e.type==='horse'?2.0:e.type==='cow'?0.9:0.5+Math.random();e.vx=(Math.random()>0.5?1:-1)*spd;if(Math.random()<0.25)e.vx=0;e.dirTimer=2000+Math.random()*4000;}if(e.type==='horse'&&e.onGround){const checkX=Math.floor((e.x+(e.vx>0?e.w+2:-2))/BS),checkY=Math.floor((e.y+e.h-4)/BS);if(solid(checkX,checkY)){e.vy=JSPD*0.8;e.onGround=false;}}}
  else if(e.type==='cat'){const dx=player.x-e.x,dist=Math.abs(dx);e.dirTimer-=dt;if(dist<120&&!player.isDead){e.vx=(dx>0?-1:1)*2.0;}else{if(e.dirTimer<=0){e.vx=(Math.random()>0.5?1:-1)*(0.6+Math.random()*0.6);if(Math.random()<0.35)e.vx=0;e.dirTimer=1500+Math.random()*3000;}}}
  else if(e.type==='dog'){const dx=player.x-e.x,dy=player.y-e.y,dist=Math.sqrt(dx*dx+dy*dy);if(dist>80&&!player.isDead){e.vx=(dx>0?1:-1)*2.5;const checkX=Math.floor((e.x+(e.vx>0?e.w+2:-2))/BS),checkY=Math.floor((e.y+e.h-4)/BS);if(solid(checkX,checkY)&&e.onGround){e.vy=JSPD*0.85;e.onGround=false;}}else if(dist<40){e.vx*=0.7;}else{e.vx*=0.5;}}
  ...
  else if(e.type==='creeper'){const dx=player.x+player.w/2-(e.x+e.w/2),dy=player.y+player.h/2-(e.y+e.h/2),dist=Math.sqrt(dx*dx+dy*dy);if(dist<350&&!player.isDead){e.vx=(dx>0?1:-1)*1.4;const checkX=Math.floor((e.x+(e.vx>0?e.w+2:-2))/BS),checkY=Math.floor((e.y+e.h/2)/BS);if(solid(checkX,checkY)&&e.onGround){e.vy=JSPD*0.7;e.onGround=false;}}else{e.vx*=0.8;}if(dist<42&&!player.isDead){if(!e.fusing){e.fusing=true;e.fuseTimer=1800;}e.fuseTimer-=dt;if(e.fuseTimer<=0){sndExplosion();for(let fdx=-3;fdx<=3;fdx++)for(let fdy=-3;fdy<=3;fdy++){if(Math.abs(fdx)+Math.abs(fdy)<=4&&getB(Math.floor(e.x/BS)+fdx,Math.floor(e.y/BS)+fdy)!==B.BEDROCK){spawnParticles(Math.floor(e.x/BS)+fdx,Math.floor(e.y/BS)+fdy,'#40A020',4);setB(Math.floor(e.x/BS)+fdx,Math.floor(e.y/BS)+fdy,B.AIR);}}if(Math.abs(player.x/BS-e.x/BS)<5&&Math.abs(player.y/BS-e.y/BS)<5){player.health=Math.max(0,player.health-25);dmgFlash=1;sndHurt();}for(const ent of entities){const edx=ent.x/BS-e.x/BS,edy=ent.y/BS-e.y/BS;if(Math.abs(edx)<5&&Math.abs(edy)<5)ent.health-=20;}e.health=0;}}else{e.fusing=false;e.fuseTimer=0;}}
  else if(e.type==='skeleton'){const dx=player.x+player.w/2-(e.x+e.w/2),dy=player.y+player.h/2-(e.y+e.h/2),dist=Math.sqrt(dx*dx+dy*dy);e.shootCD=Math.max(0,(e.shootCD||0)-dt);if(dist<350&&!player.isDead){if(dist<100){e.vx=(dx>0?-1:1)*2.0;}else if(dist>160){e.vx=(dx>0?1:-1)*1.5;}else{e.vx*=0.6;}const checkX=Math.floor((e.x+(e.vx>0?e.w+2:-2))/BS),checkY=Math.floor((e.y+e.h/2)/BS);if(solid(checkX,checkY)&&e.onGround){e.vy=JSPD*0.75;e.onGround=false;}if(e.shootCD<=0&&dist<320){const speed=4.5;const adx=player.x+player.w/2-(e.x+e.w/2),ady=player.y+player.h/2-(e.y+e.h/2);const len=Math.sqrt(adx*adx+ady*ady)||1;arrows.push({x:e.x+e.w/2,y:e.y+e.h*0.3,vx:adx/len*speed,vy:ady/len*speed-1.2});e.shootCD=2200+Math.random()*800;}}else{e.dirTimer-=dt;if(e.dirTimer<=0){e.vx=(Math.random()>0.5?1:-1)*0.8;e.dirTimer=2500+Math.random()*2500;}}}
  ...
  if(e.health<=0){const ex=Math.floor(e.x/BS),ey2=Math.floor(e.y/BS);if(e.type==='pig'){addItem('MEAT',2);addItem('LEATHER',1);sndPigDie();spawnParticles(ex,ey2,'#F0A0A0',10);}else if(e.type==='cow'){addItem('MEAT',2+Math.floor(Math.random()*2));addItem('LEATHER',1+Math.floor(Math.random()*2));sndPigDie();spawnParticles(ex,ey2,'#C07040',10);}else if(e.type==='horse'){addItem('LEATHER',2+Math.floor(Math.random()*2));sndPigDie();spawnParticles(ex,ey2,'#C8A060',10);}else if(e.type==='cat'){if(Math.random()<0.7)addItem('STRING',1);sndPigDie();spawnParticles(ex,ey2,'#F0B070',8);}else if(e.type==='dog'){if(Math.random()<0.6)addItem('BONE',1);sndPigDie();spawnParticles(ex,ey2,'#D0A060',8);}else if(e.type==='zombie'){addItem('COAL',1+Math.floor(Math.random()*2));if(Math.random()<0.15)addItem('IRON_INGOT',1);sndZombieDie();spawnParticles(ex,ey2,'#408040',10);}else if(e.type==='creeper'){addItem('COAL',1+Math.floor(Math.random()*2));spawnParticles(ex,ey2,'#40A020',10);}else if(e.type==='skeleton'){addItem('BONE',1+Math.floor(Math.random()*2));if(Math.random()<0.6)addItem('ARROW',1+Math.floor(Math.random()*2));sndZombieDie();spawnParticles(ex,ey2,'#E0E0E0',10);}entities.splice(i,1);}}}
...
function drawEntities(){for(const e of entities){const sx=e.x-cam.x|0,sy=e.y-cam.y|0;if(sx<-60||sx>W+60||sy<-60||sy>H+60)continue;const wk=Math.sin(e.walkPhase)*0.5;
if(e.type==='pig'){const dir=e.vx>=0?1:-1;ctx.fillStyle='#F0A0A0';ctx.fillRect(sx,sy+4,28,16);ctx.fillStyle='#E89090';ctx.fillRect(sx,sy+14,28,6);ctx.fillStyle='#D08080';ctx.fillRect(sx+4,sy+20,4,6+wk*3);ctx.fillRect(sx+20,sy+20,4,6-wk*3);ctx.fillRect(sx+10,sy+20,4,6-wk*3);ctx.fillRect(sx+14,sy+20,4,6+wk*3);const headX=dir>0?sx+18:sx-2;ctx.fillStyle='#F5B0B0';ctx.fillRect(headX,sy+2,12,12);ctx.fillStyle='#FFBABA';ctx.fillRect(headX+(dir>0?8:0),sy+6,4,5);ctx.fillStyle='#333';ctx.fillRect(headX+(dir>0?3:5),sy+4,2,2);ctx.fillStyle='#E09090';ctx.fillRect(dir>0?sx-2:sx+26,sy+8,4,2);ctx.fillRect(dir>0?sx-3:sx+27,sy+6,2,2);}
else if(e.type==='cow'){const dir=e.vx>=0?1:-1;ctx.fillStyle='#C07840';ctx.fillRect(sx,sy+6,34,14);ctx.fillStyle='#E8E8E8';ctx.fillRect(sx+6,sy+8,8,6);ctx.fillRect(sx+20,sy+10,7,5);ctx.fillStyle='#8A5020';ctx.fillRect(sx+4,sy+20,5,8+wk*3);ctx.fillRect(sx+25,sy+20,5,8-wk*3);ctx.fillRect(sx+11,sy+20,5,8-wk*3);ctx.fillRect(sx+18,sy+20,5,8+wk*3);const hx=dir>0?sx+24:sx-4;ctx.fillStyle='#C07840';ctx.fillRect(hx,sy+2,14,12);ctx.fillStyle='#E0A060';ctx.fillRect(hx+(dir>0?8:0),sy+8,6,6);ctx.fillStyle='#222';ctx.fillRect(hx+(dir>0?3:5),sy+4,2,2);ctx.fillStyle='#FFA0A0';ctx.fillRect(sx+12,sy+16,10,4);ctx.fillStyle='rgba(255,220,220,0.6)';ctx.fillRect(sx+14,sy+16,5,2);const hornX=dir>0?hx+2:hx+6;ctx.fillStyle='#E8D080';ctx.fillRect(hornX,sy-2,3,4);ctx.fillRect(hornX+(dir>0?1:-1),sy-4,2,2);}
else if(e.type==='horse'){const dir=e.vx>=0?1:-1;ctx.fillStyle='#C8A060';ctx.fillRect(sx,sy+6,38,20);ctx.fillStyle='#A87840';ctx.fillRect(sx,sy+16,38,10);ctx.fillStyle='#8A5A20';ctx.fillRect(sx+5,sy+26,6,10+wk*4);ctx.fillRect(sx+27,sy+26,6,10-wk*4);ctx.fillRect(sx+13,sy+26,6,10-wk*4);ctx.fillRect(sx+20,sy+26,6,10+wk*4);const hx=dir>0?sx+26:sx-2;ctx.fillStyle='#C8A060';ctx.fillRect(hx,sy+0,14,12);ctx.fillStyle='#A87840';ctx.fillRect(hx,sy+8,14,4);ctx.fillStyle='#222';ctx.fillRect(hx+(dir>0?4:6),sy+2,2,2);const maneX=dir>0?hx-2:hx+10;ctx.fillStyle='#6A3A10';ctx.fillRect(maneX,sy-4,4,14);ctx.fillStyle='#E8D080';ctx.fillRect(hx+(dir>0?10:0),sy+6,4,4);}
else if(e.type==='cat'){const dir=e.vx>=0?1:-1;ctx.fillStyle='#E8A040';ctx.fillRect(sx+2,sy+4,10,8);ctx.fillStyle='#F0B850';ctx.fillRect(sx+4,sy+6,5,4);ctx.fillStyle='#E8A040';ctx.fillRect(sx+3,sy+12,3,4+wk*2);ctx.fillRect(sx+8,sy+12,3,4-wk*2);const hx=dir>0?sx+8:sx-2;ctx.fillStyle='#E8A040';ctx.fillRect(hx,sy,8,7);ctx.fillStyle='#222';ctx.fillRect(hx+(dir>0?2:3),sy+2,2,2);ctx.fillStyle='#F0C0A0';ctx.fillRect(hx+(dir>0?4:1),sy+4,3,2);ctx.fillStyle='#E8A040';if(dir>0){ctx.fillRect(hx+5,sy-3,2,3);ctx.fillRect(hx+1,sy-3,2,3);}else{ctx.fillRect(hx+2,sy-3,2,3);ctx.fillRect(hx-2,sy-3,2,3);}const tailX=dir>0?sx-2:sx+12;ctx.fillStyle='#C87030';ctx.fillRect(tailX,sy+4,2,6);ctx.fillRect(tailX+(dir>0?-2:2),sy,2,5);}
else if(e.type==='dog'){const dir=e.vx>=0?1:-1;ctx.fillStyle='#C88040';ctx.fillRect(sx+2,sy+4,16,10);ctx.fillStyle='#E0A060';ctx.fillRect(sx+4,sy+6,8,6);ctx.fillStyle='#A06020';ctx.fillRect(sx+3,sy+14,4,5+wk*2);ctx.fillRect(sx+13,sy+14,4,5-wk*2);const hx=dir>0?sx+14:sx-2;ctx.fillStyle='#C88040';ctx.fillRect(hx,sy+1,8,8);ctx.fillStyle='#E0A060';ctx.fillRect(hx+(dir>0?4:0),sy+4,4,4);ctx.fillStyle='#222';ctx.fillRect(hx+(dir>0?2:3),sy+2,2,2);ctx.fillStyle='#C88040';const earX=dir>0?hx+3:hx+1;ctx.fillRect(earX,sy-2,4,4);const tailX=dir>0?sx-3:sx+18;ctx.fillStyle='#A06020';ctx.fillRect(tailX,sy+2,3,5);ctx.fillRect(tailX+(dir>0?-2:2),sy,3,4);}
else if(e.type==='zombie'){const dir=e.vx>=0?1:-1,atkFrac=Math.max(0,e.attackTimer/400),lungeX=atkFrac*dir*6,drawX=sx+lungeX;ctx.fillStyle='#3A3A5E';ctx.fillRect(drawX+4,sy+16,14,20);ctx.fillStyle='#2A2A4E';ctx.fillRect(drawX+6,sy+22,4,10);ctx.fillStyle='#3A3A5E';ctx.save();ctx.translate(drawX+8,sy+36);ctx.rotate(wk);ctx.fillRect(-3,0,6,16);ctx.restore();ctx.save();ctx.translate(drawX+14,sy+36);ctx.rotate(-wk);ctx.fillRect(-3,0,6,16);ctx.restore();if(atkFrac>0.1){ctx.fillStyle='#4E8A4E';ctx.fillRect(drawX+(dir>0?-2:16),sy+18,8,10);ctx.fillRect(drawX+(dir>0?14:-8-atkFrac*6),sy+14,12+atkFrac*4,7);}else{ctx.fillStyle='#4E8A4E';ctx.fillRect(drawX+(dir>0?-2:16),sy+18,8,6);ctx.fillRect(drawX+(dir>0?14:-8),sy+16,12,6);}ctx.fillStyle='#4E8A4E';ctx.fillRect(drawX+3,sy,16,16);ctx.fillStyle='#3E7A3E';ctx.fillRect(drawX+3,sy+12,16,4);ctx.fillStyle=`rgba(255,0,0,${atkFrac>0.1?1:0.8})`;ctx.fillRect(drawX+5,sy+5,3,3);ctx.fillRect(drawX+12,sy+5,3,3);if(atkFrac>0.3){const slX=drawX+(dir>0?e.w+2:-20),slY=sy+15;ctx.strokeStyle=`rgba(255,30,30,${atkFrac*0.9})`;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(slX,slY);ctx.lineTo(slX+dir*14,slY-8);ctx.moveTo(slX,slY+6);ctx.lineTo(slX+dir*12,slY+12);ctx.stroke();ctx.lineWidth=1;}}
else if(e.type==='creeper'){const flashOn=e.fusing&&Math.floor(Date.now()/150)%2===0;const bodyCol=flashOn?'#FFFFFF':'#38A030';const faceCol=flashOn?'#AAFFAA':'#1A6018';ctx.fillStyle=bodyCol;ctx.fillRect(sx+2,sy+10,14,14);ctx.fillRect(sx+2,sy+24,5,10+wk*3);ctx.fillRect(sx+11,sy+24,5,10-wk*3);ctx.fillStyle=bodyCol;ctx.fillRect(sx+3,sy,12,12);ctx.fillStyle=faceCol;ctx.fillRect(sx+4,sy+3,3,3);ctx.fillRect(sx+11,sy+3,3,3);ctx.fillRect(sx+6,sy+7,6,2);ctx.fillRect(sx+5,sy+9,2,3);ctx.fillRect(sx+11,sy+9,2,3);}
else if(e.type==='skeleton'){const dir=e.vx>=0?1:-1;ctx.fillStyle='#D8D8C8';ctx.fillRect(sx+4,sy,10,10);ctx.fillStyle='#222';ctx.fillRect(sx+5,sy+2,3,3);ctx.fillRect(sx+10,sy+2,3,3);ctx.fillRect(sx+6,sy+7,6,2);ctx.fillStyle='#D8D8C8';ctx.fillRect(sx+5,sy+10,8,14);ctx.fillStyle='#C0C0B0';ctx.fillRect(sx+6,sy+12,6,3);ctx.fillRect(sx+6,sy+18,6,2);ctx.fillStyle='#D8D8C8';ctx.save();ctx.translate(sx+6,sy+12);ctx.rotate(wk*0.5);ctx.fillRect(-2,0,3,10);ctx.restore();ctx.save();ctx.translate(sx+12,sy+12);ctx.rotate(-wk*0.5);ctx.fillRect(-1,0,3,10);ctx.restore();ctx.save();ctx.translate(sx+5,sy+24);ctx.rotate(wk*0.4);ctx.fillRect(-2,0,4,14);ctx.restore();ctx.save();ctx.translate(sx+11,sy+24);ctx.rotate(-wk*0.4);ctx.fillRect(-2,0,4,14);ctx.restore();const bowX=dir>0?sx-4:sx+e.w;ctx.strokeStyle='#8B6914';ctx.lineWidth=2;ctx.beginPath();ctx.arc(bowX,sy+14,8,dir>0?-Math.PI*0.7:Math.PI*0.3,dir>0?Math.PI*0.7:Math.PI*1.7);ctx.stroke();ctx.strokeStyle='#555';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(bowX,sy+6);ctx.lineTo(bowX,sy+22);ctx.stroke();ctx.lineWidth=1;}
if(e.hitFlash>0.05){ctx.fillStyle=`rgba(255,255,255,${e.hitFlash*0.6})`;ctx.fillRect(sx,sy,e.w,e.h);}if(e.health<e.maxHealth){const bw=e.w,frac=e.health/e.maxHealth;ctx.fillStyle='rgba(0,0,0,0.6)';ctx.fillRect(sx,sy-8,bw,5);ctx.fillStyle='#E02020';ctx.fillRect(sx+1,sy-7,(bw-2)*frac,3);}}}
function updateArrows(dt){const s=dt/16.667;for(let i=arrows.length-1;i>=0;i--){const a=arrows[i];a.vy+=GRAV*s*0.28;a.x+=a.vx*s;a.y+=a.vy*s;const tx=Math.floor(a.x/BS),ty=Math.floor(a.y/BS);if(solid(tx,ty)){arrows.splice(i,1);continue;}if(!player.isDead&&a.x>player.x-4&&a.x<player.x+player.w+4&&a.y>player.y-4&&a.y<player.y+player.h+4){let dmg=5-getArmorDef();dmg=Math.max(1,dmg);player.health=Math.max(0,player.health-dmg);dmgFlash=1;sndHurt();arrows.splice(i,1);continue;}if(Math.abs(a.x-player.x)>1500||a.y>WH*BS){arrows.splice(i,1);}}}
function drawArrows(){for(const a of arrows){const sx=(a.x-cam.x)|0,sy=(a.y-cam.y)|0;if(sx<-20||sx>W+20)continue;const angle=Math.atan2(a.vy,a.vx);ctx.save();ctx.translate(sx,sy);ctx.rotate(angle);ctx.fillStyle='#8B6914';ctx.fillRect(-8,-1,16,2);ctx.fillStyle='#C8A020';ctx.fillRect(6,-2,5,4);ctx.fillStyle='#EEEECC';ctx.fillRect(-10,-2,4,4);ctx.restore();}}

const keys={};let mouseWX=0,mouseWY=0,mouseX=0,mouseY=0;let lmbDown=false,rmbDown=false;let mineT=0,mineTarget=null,hitCD=0;

const chatMessages=[];
let chatOpen=false;
let chatAnnouncedDeath=false;
function chatDisplayName(name){return (name||'Player').trim()||'Player';}
function pushChatMessage(text,type='system',name=''){
  if(!text) return;
  chatMessages.push({text:String(text),type,name:name||'',time:Date.now()});
...
function renderChatLog(){
  ...
  const now=Date.now();
  log.innerHTML='';
  const ICONS={join:'▶ ',leave:'◀ ',death:'✦ ',system:'● ',chat:''};
  const NAME_COLORS=['#55ffff','#ffff55','#ff55ff','#55ff55','#ffaa55','#aaaaff'];
  let nameIdx=0;
  chatMessages.slice(-8).forEach(m=>{
    const el=document.createElement('div');
    const age=now-m.time;
    const SHOW_SECS=chatOpen?999999:15;
    const FADE_SECS=3;
    const ageSec=age/1000;
    let opacity=1;
    if(!chatOpen){
      if(ageSec>SHOW_SECS) opacity=0;
      else if(ageSec>(SHOW_SECS-FADE_SECS)) opacity=Math.max(0,(SHOW_SECS-ageSec)/FADE_SECS);
    }
    el.className='chat-line '+(m.type||'system');
    el.style.opacity=opacity.toString();
    const type=m.type||'system';
    const icon=ICONS[type]||'';
    let html='';
    if(type==='chat'&&m.name){
      const col=NAME_COLORS[(nameIdx++)%NAME_COLORS.length];
      html='<span class="chat-name" style="color:'+col+';">&lt;'+escapeHtml(m.name)+'&gt;</span> '+escapeHtml(m.text);
    } else {
      html=escapeHtml(icon+(m.text||''));
    }
    el.innerHTML=html;
    log.appendChild(el);
  });
...
function setChatOpen(open){
  ...
  if(wrap) wrap.style.display=chatOpen?'flex':'none';
  if(chatOpen){ if(inp){ inp.value=''; inp.focus(); } }
  else { if(inp) inp.blur(); }
  renderChatLog();
}
function submitChat(){
  const inp=document.getElementById('chat-input');
  if(!inp){ setChatOpen(false); return; }
  const msg=inp.value.trim();
  setChatOpen(false);
  if(!msg) return;
  if(typeof sendChatMessage === 'function') sendChatMessage(msg, playerName);
}
function openChat(){
  if(gameState!=='playing' || player.isDead) return;
  if(craftOpen || furnaceOpen || chestOpen || bookOpen) return;
  setChatOpen(true);
}
...
function doRightClick(){if(craftOpen||furnaceOpen||chestOpen||player.isDead)return;const sl=inv[hotIdx];if(!sl)return;const it=IT[sl.key];if(it?.food){if(player.hunger<player.maxHunger||player.health<player.maxHealth){player.hunger=Math.min(player.maxHunger,player.hunger+(it.hunger||20));player.health=Math.min(player.maxHealth,player.health+(it.heal||5));if(worldMode!=='creative'){if(worldMode!=='creative'){sl.count--;if(sl.count<=0)inv[hotIdx]=null;}}isEating=true;eatTimer=0;eatColor=it.c;sndEat();return;}}const{tx,ty}=getTile();if(!inReach(tx,ty))return;
  if(isDoorBlock(getB(tx,ty))||isDoorTopBlock(getB(tx,ty))){if(toggleDoorPair(tx,ty))return;}
  if(getB(tx,ty)===B.BENCH){craftOpen=true;checkCraftRecipe();return;}if(getB(tx,ty)===B.FURNACE){openFurnace();return;}if(getB(tx,ty)===B.CHEST){openChest(tx,ty);return;}if(getB(tx,ty)===B.BED){if(dayT>0.86||dayT<0.14){dayT=0.18;sndLevelUp();spawnWorldParticles(player.x+player.w/2,player.y,'#FFFF88',8);return;}}if(it?.block===B.DOOR){if(placeDoorPair(tx,ty)) {if(worldMode!=='creative'){sl.count--;if(sl.count<=0)inv[hotIdx]=null;}isPlacing=true;placeTimer=0;placeEffects.push({x:tx,y:ty,life:1});}return;}if(it?.block===undefined)return;if(getB(tx,ty)!==B.AIR)return;if(tx*BS<player.x+player.w&&(tx+1)*BS>player.x&&ty*BS<player.y+player.h&&(ty+1)*BS>player.y)return;sndPlace();setB(tx,ty,it.block);if(onlineMode)broadcastToAll({type:'block',x:tx,y:ty,t:it.block});if(worldMode!=='creative'){sl.count--;if(sl.count<=0)inv[hotIdx]=null;}isPlacing=true;placeTimer=0;placeEffects.push({x:tx,y:ty,life:1});}

function physicsStep(dt){if(player.isDead)return;const s=dt/16.667;player.vx=0;if(!craftOpen&&!furnaceOpen&&!chestOpen){if(bookOpen){if(keys['KeyA']&&bookPage>0){bookFlipDir=-1;bookFlipAnim=1.0;bookPage--;keys['KeyA']=false;}if(keys['KeyD']&&bookPage<BOOK_PAGES.length-1){bookFlipDir=1;bookFlipAnim=1.0;bookPage++;keys['KeyD']=false;}return;}
